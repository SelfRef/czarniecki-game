...
void bintohex(const void* input, char* output, unsigned int inputlen) {
...
void hextobin(const char* input, void* output, unsigned int outputlen) {
	for(unsigned int i = 0; i < outputlen; ++i) {
		char a = input[2 * i];
		char b = input[2 * i + 1];
		((unsigned char*)(output))[i] =
			((unsigned char)(('0' <= a && a <= '9')? a - '0' : (('a' <= a && a <= 'f')? 10 + a - 'a' : (('A' <= a && a <= 'F')? 10 + a - 'A' : 0))) << 4) |
			(unsigned char)(('0' <= b && b <= '9')? b - '0' : (('a' <= b && b <= 'f')? 10 + b - 'a' : (('A' <= b && b <= 'F')? 10 + b - 'A' : 0)));
	}
}
