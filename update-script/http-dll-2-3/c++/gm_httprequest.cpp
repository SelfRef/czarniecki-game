...
gmexport const char* httprequest_get_response_header_name(double id, double index) {
	HttpRequest *hr = gmdata.FindHttpRequest(gm_cast<unsigned int>(id));
	if(hr == NULL) return "";
	std::string str = hr->GetResponseHeaderName(gm_cast<unsigned int>(index));
	gmreturnstring.SetLength(str.length());
	memcpy(gmreturnstring.GetData(), str.data(), str.length());
...
gmexport const char* httprequest_get_response_header_value(double id, double index) {
	HttpRequest *hr = gmdata.FindHttpRequest(gm_cast<unsigned int>(id));
	if(hr == NULL) return "";
	std::string str = hr->GetResponseHeaderValue(gm_cast<unsigned int>(index));
	gmreturnstring.SetLength(str.length());
	memcpy(gmreturnstring.GetData(), str.data(), str.length());
	return gmreturnstring.ToString();
}
...
gmexport const char* httprequest_find_response_header(double id, const char* name) {
	HttpRequest *hr = gmdata.FindHttpRequest(gm_cast<unsigned int>(id));
	if(hr == NULL) return "";
	std::string str = hr->FindResponseHeader(name);
	gmreturnstring.SetLength(str.length());
	memcpy(gmreturnstring.GetData(), str.data(), str.length());
	return gmreturnstring.ToString();
}
...
gmexport const char* httprequest_get_message_body(double id) {
	...
	memcpy(gmreturnstring.GetData(), hr->GetMessageBody(), l);
	return gmreturnstring.ToString();
}
...
gmexport const char* httprequest_urlencode(const char* string, double keepspecialchars) {
	std::string str = HttpRequest::UrlEncode(string, gm_cast<bool>(keepspecialchars));
	gmreturnstring.Clear();
	gmreturnstring.SetLength(str.length());
	memcpy(gmreturnstring.GetData(), str.data(), str.length());
	return gmreturnstring.ToString();
}
