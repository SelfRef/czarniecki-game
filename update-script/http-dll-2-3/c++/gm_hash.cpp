...
gmexport const char* md5_result() {
	gmreturnstring.SetLength(32);
	bintohex(gm_md5.Result(), gmreturnstring.GetData(), 16);
...
gmexport const char* sha1_result() {
	gmreturnstring.SetLength(40);
	bintohex(gm_sha1.Result(), gmreturnstring.GetData(), 20);
	return gmreturnstring.ToString();
}
